...
parser.add_argument("-s",  "--seed",         default=2,   type=int,   help="Set the random seed.")
...
def predict_fn(model, batch) -> tuple[Tensor, Tensor, Tensor]:
    ...
    BS, timesteps, dim = Y.shape
    dummy_YMASK = MY.clone()
    dummy_YMASK[:] = 0
    dummy_YMASK[:, -1, :] = 1
    output, target_U_, target_mask_ = model(T, X, M, TY, Y, dummy_YMASK)
    op = torch.stack([o[m] for o, m in zip(output.squeeze(), target_mask_)])
    YHAT = MY * op.unsqueeze(1)
    return Y, YHAT, MY.to(torch.bool)
