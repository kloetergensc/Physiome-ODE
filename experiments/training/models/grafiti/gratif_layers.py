...
from models.grafiti.attention import MAB2, indMAB
...
class bipartitegraph_encoder(nn.Module):
    def __init__(
        ...
        value_dims = (
            1 + 1 + tim_dims + dim
        )  # valuedims(1), targetindicator(1), timeembed(tim_dims), channelembed(#channels)

        for i in range(self.n_layers):
            self.attn_blocks.append(
                indMAB(induced_dims, value_dims, nkernel, self.nheads)
            )
            induced_dims = nkernel
            value_dims = nkernel
        self.relu = nn.ReLU()
        temp = torch.arange(self.n_induced_points, dtype=torch.int64)[None, :].to(
            self.device
        )
        self.induced_points = torch.nn.functional.one_hot(
            temp, num_classes=self.n_induced_points
        ).to(torch.float32)
    ...
    def forward(self, context_x, value, mask, target_value, target_mask):
        ...
        T = context_x[:, :, None].repeat(
            1, 1, ndims
        )  # BxTxC, observed time points in the input
        C_inds = (
            torch.cumsum(torch.ones_like(value).to(torch.int64).to(self.device), -1) - 1
        )  # BxTxC init for channel indices
        mk_bool = mask.to(torch.bool)  # BxTxC boolean mask for the input
        full_len = torch.max(mask.sum((1, 2))).to(
            torch.int64
        )  # flattened TxC max length possible
        pad = lambda v: F.pad(
            v, [0, full_len - len(v)], value=0
        )  # defined function for padding the sequences
        ...
        T_ = torch.stack(
            [pad(r[m]) for r, m in zip(T, mk_bool)]
        ).contiguous()  # BxTxC (Times) -> Bxfull_len , flattened time
        U_ = torch.stack(
            [pad(r[m]) for r, m in zip(value, mk_bool)]
        ).contiguous()  # BxTxC (values) -> Bxfull_len, flattened values
        target_U_ = torch.stack(
            [pad(r[m]) for r, m in zip(target_value, mk_bool)]
        ).contiguous()  # BxK_, flattened target values
        target_mask_ = torch.stack(
            [pad(r[m]) for r, m in zip(target_mask, mk_bool)]
        ).contiguous()  # BxK_, flattened target mask
        C_inds_ = torch.stack(
            [pad(r[m]) for r, m in zip(C_inds, mk_bool)]
        ).contiguous()  # BxK_, flattened channel indices
        mk_ = torch.stack(
            [pad(r[m]) for r, m in zip(mask, mk_bool)]
        ).contiguous()  # BxK_, flattened mask

        C_ = torch.nn.functional.one_hot(C_inds_.to(torch.int64), num_classes=ndims).to(
            torch.float32
        ...
        U_ = torch.cat(
            [T_emb, C_, U_[:, :, None], U_indicator[:, :, None]], -1
        )  # BxK_max x 3 , OHE of channels, values and indicators
        ...
        for i in range(self.n_layers):
            induced_input, att_input = self.attn_blocks[i](
                induced_input, att_input, att_mask
            )  # performing induced multihead attention
            att_input *= mk_[:, :, None].repeat(
                1, 1, self.nkernel
            )  # multiplying with the mask
        output = self.out_layer(
            att_input
        )  # passing attention outputs to the linear layer
        return output.squeeze(-1), target_U_, target_mask_


class fullgraph_encoder(nn.Module):
    def __init__(
        self, dim=41, tim_dims=64, nkernel=128, n_layers=3, attn_head=4, device="cuda"
    ):
    ...
    def forward(self, context_x, value, mask, target_value, target_mask):
        ...
        for i in range(self.n_layers):
            att_input = self.attn_blocks[i](
                att_input, att_input, att_mask
            )  # performing induced multihead attention
            att_input *= mk_[:, :, None].repeat(
                1, 1, self.nkernel
            )  # multiplying with the mask
    ...
    def __init__(self, dim=41, nkernel=128, n_layers=3, attn_head=4, device="cuda"):
        ...
        for i in range(self.n_layers):
            ...
            self.channel_attn.append(
                MAB2(nkernel, nkernel, nkernel, nkernel, self.nheads)
            )
        self.relu = nn.ReLU()
