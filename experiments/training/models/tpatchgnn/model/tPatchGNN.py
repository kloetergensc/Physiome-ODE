...
from models.tpatchgnn.model.SelfAttention_Family import AttentionLayer, FullAttention
from models.tpatchgnn.model.Transformer_EncDec import Encoder, EncoderLayer
...
class tPatchGNN(nn.Module):
    def __init__(self, args, supports=None, dropout=0):
        ...
        self.nodevec1 = nn.Parameter(
            torch.randn(self.N, nodevec_dim).cuda(), requires_grad=True
        )
        self.nodevec2 = nn.Parameter(
            torch.randn(nodevec_dim, self.N).cuda(), requires_grad=True
        )
        ...
        for _ in range(self.n_layer):
            ...
            self.nodevec_gate1.append(
                nn.Sequential(
                    nn.Linear(args.hid_dim + nodevec_dim, 1), nn.Tanh(), nn.ReLU()
                )
            )
    ...
    def IMTS_Model(self, x, mask_X):
        ...
        x_patch = x_patch.view(
            self.batch_size, self.N, self.M, -1
        )  # (B, N, M, hid_dim)
        ...
        for layer in range(self.n_layer):
            ...
            x_gate1 = self.nodevec_gate1[layer](
                torch.cat([x, nodevec1.permute(0, 2, 1, 3)], dim=-1)
            )
            x_gate2 = self.nodevec_gate2[layer](
                torch.cat([x, nodevec2.permute(0, 3, 1, 2)], dim=-1)
            )
            ...
            adp = F.softmax(
                F.relu(torch.matmul(nodevec1, nodevec2)), dim=-1
            )  # (B, M, N, N) used
        ...
        if self.outlayer == "CNN":
            x = x.reshape(self.batch_size * self.N, self.M, -1).permute(
                0, 2, 1
            )  # (B*N, F, M)
    ...
    def forecasting(self, time_steps_to_predict, X, truth_time_steps, mask=None):
        ...
        truth_time_steps = truth_time_steps.permute(0, 3, 1, 2).reshape(
            -1, L_in, 1
        )  # (B*N*M, L, 1)
        ...
        time_steps_to_predict = time_steps_to_predict.view(B, 1, L_pred, 1).repeat(
            1, N, 1, 1
        )  # (B, N, Lp, 1)
    ...
    def forecasting_with_targets(self, batch_dict):
        time_steps_to_predict = batch_dict["tp_to_predict"].to(self.device)
        X = batch_dict["observed_data"].to(self.device)
        truth_time_steps = batch_dict["observed_tp"].to(self.device)
        mask = batch_dict["observed_mask"].to(self.device)
        return (
            self.forecasting(time_steps_to_predict, X, truth_time_steps, mask),
